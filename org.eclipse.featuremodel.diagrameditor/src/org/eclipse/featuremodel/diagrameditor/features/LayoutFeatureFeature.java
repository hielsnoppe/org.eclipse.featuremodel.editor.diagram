...
public class LayoutFeatureFeature extends AbstractLayoutFeature {
  ...
  /**
   * Layouts only the given Feature pictogram element. The Feture pictogram
   * element to layout is positioned under the parent Feature or rightmost of
   * all child Features.
   * 
   * @param context
   *          The context.
  ...
  public boolean layout(ILayoutContext context) {
    ...
    if (parent != null) {
      ...
      ContainerShape peParent = BOUtil.getPictogramElementForBusinessObject(parent, ContainerShape.class, getFeatureProvider());
  ...
  /**
   * Determines the x coordinate of the rightmost child Feature pictogram
   * element of the given parent Feature.
   * 
   * @param parent
   *          The parent Feature
    ...
    for (Group g : parent.getChildren()) {
      for (Feature f : g.getFeatures()) {
        if (!f.equals(featureToLayout)) {
          ContainerShape cs = BOUtil.getPictogramElementForBusinessObject(f, ContainerShape.class, getFeatureProvider());
