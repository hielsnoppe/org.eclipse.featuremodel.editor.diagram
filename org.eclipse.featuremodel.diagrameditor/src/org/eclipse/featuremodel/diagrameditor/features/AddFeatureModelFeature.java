...
/**
 * Feature handle adding to a diagram a graphical representation of an existing
...
public class AddFeatureModelFeature extends AbstractAddFeature {
  ...
  public boolean canAdd(IAddContext context) {
    // This feature supports to add a 'Feature Model' object directly into the
    // diagram
  ...
  /**
   * Adds a graphical representation of an existing Feature Model to the
   * diagram.
   * 
   * @param context
   *          The context.
  ...
  private void createFeatureModelTree(Feature parentFeature) {
    ContainerShape parentContainer = (ContainerShape) this.getFeatureProvider().getPictogramElementForBusinessObject(parentFeature);
    ...
    for (Group group : parentFeature.getChildren()) {
      ...
      for (Feature feature : group.getFeatures()) {
        ...
        ContainerShape cs = BOUtil.getPictogramElementForBusinessObject(feature, ContainerShape.class, getFeatureProvider());
