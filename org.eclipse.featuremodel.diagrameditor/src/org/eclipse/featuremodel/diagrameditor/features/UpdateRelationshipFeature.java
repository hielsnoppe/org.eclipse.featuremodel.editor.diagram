...
public class UpdateRelationshipFeature extends AbstractUpdateFeature {
   ...
   * <ul>
   * <li>Update of the graphical representation of Group according to actual
   * capacity of Group (single or set relation).
   * <li>Update of the location and form of set relation pictogram element.
   * <li>Update look of the given pictogram element for Group according to the
  ...
  /**
   * Updates the set relation. Creates pictogram element and graphical
   * representation of set relation if not exist, otherwise update the location
   * and form of the pictogram element and graphical representation.
   * 
   * @param group
   *          The Group to update.
   */
  ...
  /**
   * Updates the single relation. Creates pictogram element and graphical
   * representation of single relation if not exist.
   * 
   * @param group
   *          The Group to update.
   */
  private void updateSingleRelation(Group group) {
    ...
    Connection connection = BOUtil.getPictogramElementForBusinessObject(group, Connection.class, getFeatureProvider());
    ...
    if (cd == null) {
      ...
      Graphiti.getPeService().setPropertyValue(cd, Properties.PROP_KEY_RELATION_TYPE, Properties.PROP_VAL_RELATION_TYPE);
  ...
  private void createSetRelationGraphic(Group group, ContainerShape pe) {
    ...
    // calculate new polygon points locations according to position of two outer
    // connections
  ...
  private ConnectionDecorator getSingleRelationDecorator(Connection connection) {
    ...
    for (ConnectionDecorator cd : connection.getConnectionDecorators()) {
      for (Property p : cd.getProperties()) {
        if (Properties.PROP_KEY_RELATION_TYPE.equals(p.getKey()) && Properties.PROP_VAL_RELATION_TYPE.equals(p.getValue())) {
          result = cd;
          break;
        }
      }
  ...
  /**
   * Deletes the pictogram element and the graphical representation of set
   * relation.
   * 
   * @param group
  ...
  /**
   * Deletes the pictogram element and the graphical representation of single
   * relation.
   * 
   * @param group
   ...
   * means whether the graphics algorithm of this pictogram element contain the
   * latest values from the business objects.
   * 
   * @param context
   *          The context.
   * 
