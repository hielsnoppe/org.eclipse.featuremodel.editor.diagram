...
/**
 * Feature handle adding to the diagram a graphical representation of an
 * existing Feature model object.
 * 
 */
public class AddFeatureFeature extends AbstractAddFeature {
  ...
  /**
   * Checks whether a pictogram element of the given Feature Model object can be
  ...
  /**
   * Adds a graphical representation of an existing Feature model object to the
   * diagram.
   * 
   * @param context
   *          The context.
   ...
   * {@link CollapseFeatureFeature}) to hide the child elements and expanded
   * (see {@link ExpandFeatureFeature}) to show this. The expand sign is only
   * shown by Features with child Features.
   * 
   * @param featureContainerShape
   *          the main container shape of the Feature
  ...
  private ContainerShape drawExpadSign(ContainerShape featureContainerShape) {
    ...
    peService.setPropertyValue(expandSignContainer, Properties.PROP_KEY_CONTAINER_TYPE, Properties.PROP_VAL_CONTAINER_TYPE_EXPANDSIGN);
