...
/**
 * Feature handle expanding of Feature Diagram elements. A Features with child
 * Features can be collapsed (see {@link CollapseFeatureFeature}) to hide the
 * child elements and expanded (see {@link ExpandFeatureFeature}) to show this.
 * 
 */
public class ExpandFeatureFeature extends AbstractCustomFeature {
  ...
  /**
   * Decides if the expand feature is available with the given context. This
   * implementation returns <code>true</code> if the selected pictogram element
   ...
   * expanded. This implementation returns <code>true</code> if the selected
   * pictogram element represents a Feature, it has child Features and is of the
   * type {@link Properties#PROP_VAL_CONTAINER_TYPE_COLLAPSED}.
   * 
   * @param context
   *          The context.
   * @return true if the selected pictogram element represents a Feature, it has
   *         child Features and is of the type
  ...
  public void execute(ICustomContext context) {
    ...
    // collapse all child elements and update child positions by the given
    // deltas.
    expand(feature, deltaX, deltaY);

    // set the type of the given Feature pictogram element to expanded
    Graphiti.getPeService().setPropertyValue(selectedPE, Properties.PROP_KEY_CONTAINER_TYPE, Properties.PROP_VAL_CONTAINER_TYPE_EXPANDED);
  ...
  /**
   * Expand recursively all child elements of the given Feature and update child
   * positions by the given deltas.
   * 
   * @param parent
   *          the parent Feature
   * @param deltaX
  ...
  private void expand(Feature parent, int deltaX, int deltaY) {
    ...
    for (Group grp : parent.getChildren()) {
      ...
      for (Feature child : grp.getFeatures()) {
        ...
        // if a child Feature was already collapsed don't expand his child
        // Features
   ...
   * @param feature
   *          the Feature
   * @return <code>true</code> if the pictogram element of the given Feature is
   *         collapsed.
   */
