...
public class MoveFeatureFeature extends DefaultMoveShapeFeature {
  ...
  public boolean canMoveShape(IMoveShapeContext context) {
    ...
    if (objToMove instanceof Feature) {
      ...
      if (target instanceof Group //
        ...
        // allow if the target is a Feature and not a child of the Feature to
        // move
        return true;
      }
   ...
   * the Feature to move will be reconnected, otherwise only its position in the
   * diagram is changed.
   * 
   * @param context
   *          the context
  ...
  public void internalMove(IMoveShapeContext context) {
    ...
    if (newParent instanceof Feature || newParent instanceof Group) {
      Feature featureToMove = (Feature) this.getFeatureProvider().getBusinessObjectForPictogramElement(context.getShape());
  ...
  /**
   * Update all associated Groups after moving the Feature pictogram element. A
   * Feature is associated with the parent Group and child Groups.
   * 
   * @param context
   *          The context.
   */
  ...
  private void moveChildElements(Feature parent, int deltaX, int deltaY) {
    for (Group grp : parent.getChildren()) {
      for (Feature child : grp.getFeatures()) {
        ContainerShape shape = BOUtil.getPictogramElementForBusinessObject(child, ContainerShape.class, getFeatureProvider());
  ...
  private void removeParentGroup(Feature featureToMove) {
    ...
    if (oldParentGroup.getFeatures().size() == 0) {
      ...
      ContainerShape featureToMoveCS = BOUtil.getPictogramElementForBusinessObject(featureToMove, ContainerShape.class, getFeatureProvider());
  ...
  private void drawGroup(Group group, Feature childFeature, Feature parentFeature) {
    ...
    ContainerShape newFeatureCS = BOUtil.getPictogramElementForBusinessObject(childFeature, ContainerShape.class, getFeatureProvider());
